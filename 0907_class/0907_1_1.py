...
#두 조건을 동시에 만족하는 행(35세 이상, 여성) 
    st.subheader("3) 35세 이상 여성 승객의 수")
    over_35_female = df[(df["Age"] >= 35) & (df["Sex"] == "female")]
    st.write(f"나이 35세 이상 & 여성 승객 수 :**{len(over_35_female)}명**") 

    st.markdown("---")

    # 나이의 결측치 확인 및 dropna 처리

    st.subheader("4) Age 결측치 처리")
    missing_age_count = df["Age"].isna().sum() #isna는 결측치면 True 반환
    ...
    df_clean.to_csv(output_path,index=False) #원래 인덱스는 가져오지 않겠다는 뜻
